...
//		s.setBackgroundTransparency(0.7);  // Default is 0, meaning background is not transparent.
	...
	private void doMenuCommand(String command) {
		if (command.equals("Save...")) { // save all the string info to a file
			...
			File textFile = fileChooser.getOutputFile(this, "Select Text File Name", "info.txt");
			if (textFile == null)
				return;
			try {
				...
				for(DrawTextItem i: theString) {
					writer.println(i.getString());
					writer.println(i.getTextColor().getRed());
					writer.println(i.getTextColor().getGreen());
					writer.println(i.getTextColor().getBlue());
				}
				writer.close();
			}
			catch (Exception e) {
				JOptionPane.showMessageDialog(this, 
						"Sorry, an error occurred while trying to save the text info:\n" + e);
			}
		}
			...
			Color bck = new Color(Integer.parseInt(in.nextLine()),Integer.parseInt(in.nextLine()),Integer.parseInt(in.nextLine()) );
			...
			while (in.hasNextLine()) {
				...
				Color textC = new Color(Integer.parseInt(in.nextLine()),Integer.parseInt(in.nextLine()),Integer.parseInt(in.nextLine()));
				DrawTextItem item = new DrawTextItem(s, x, y);
				item.setTextColor(textC);
				theString.add(item);
			}
			canvas.repaint();
			}
